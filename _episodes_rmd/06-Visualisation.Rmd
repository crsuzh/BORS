...
```{r, fig.show="hold", out.width="50%", fig.height=4, fig.width=4}
set.seed(123)
...
```{r, fig.show="hold", out.width="33%", fig.height=4, fig.width=4}
for (i in 1:3) {
...
```{r, warning=FALSE, fig.show="hold", out.width="50%", fig.height=5, fig.width=5}
ggplot(df) +
...
```{r, fig.show="hold", out.width="50%", fig.height=5, fig.width=5}
ggplot(df) +
  geom_point(aes(x,y)) +
  coord_fixed(ratio=1)+
  medtheme()
...
ggplot(climatedf_comp_translated) +
  geom_line(aes(Year, Annual_temperature, color=Altitude,group=Location)) +
  # facet_wrap(~Location)+
  # geom_label(aes(Year,Annual_temperature,label=Location),data = climatedf_comp_translated %>% 
  # dplyr::filter(Location %in% c("ZürichFluntern","Säntis","Samedan","LocarnoMonti")) %>% 
  #   dplyr::filter(Year==min(Year)+5),nudge_y = 1) +
...
```{r, include=show_results}
climatedf_comp %>% 
  dplyr::select(Location, Year, Sunshine_duration, Annual_frost_days, Annual_summer_days) %>% 
...
```{r, include=show_results}
climatedf_comp_red <- climatedf_comp[climatedf_comp$Location %in% c("Luzern","ZürichFluntern","Lugano"),]
colors_use <- RColorBrewer::brewer.pal(length(unique(climatedf_comp_red$Location)), "Set2")
