---
source: Rmd
title: "Reproducible and honest visualizations"
teaching: 90
exercises: 90-120
questions: 
...
```{r, fig.height=2.5, fig.width=2.5, fig.cap=""}
for (i in 1:3) {
...
```{r, fig.height=6, fig.width=6, fig.cap=""}
set.seed(123)
...
```{r histo_only, fig.height=3, fig.width=3, fig.cap=""}
ggplot(df_long) +
...
df_long %>% 
  ...
  dplyr::summarise(q25 = round(quantile(y, probs = 0.25),1),
                   q50 = round(quantile(y, probs = 0.5),1),
                   q75 = round(quantile(y, probs = 0.75),1)) %>% 
  knitr::kable()
```
...
The code for the above plot:
```{r, eval=FALSE, echo=TRUE, fig.height=3, fig.width=3, fig.cap=""}
ggplot(df_long) +
  geom_violin(aes(y = y, x = dataset)) +
  geom_jitter(aes(y = y, x = dataset), width = 0.3)
```
...
```{r, warning=FALSE, fig.height=3, fig.width=3, fig.cap=""}
ggplot(df) +
...
```{r, fig.height=3, fig.width=5, fig.cap=""}
set.seed(123)
...
```{r, fig.height=4, fig.width=4, fig.cap=""}
mtcars %>% 
  rownames_to_column() %>% 
  ggplot() +
...
```{r}
climatedf_comp <- read.csv(here::here("../files/docs/06/climate_data.csv"))
```
...
>## Solution
> ```{r, include=TRUE, echo=TRUE, fig.cap=""}
> climatedf_comp %>% 
...
>## Solution
> ```{r, include=TRUE, echo=TRUE, warning=FALSE, fig.cap=""}
> climatedf_comp %>% 
>   dplyr::filter(Location %in% c("ZürichFluntern",
>                                 "Säntis",
>                                 "Samedan",
...
```{r, eval=TRUE,echo=show_results, warning=FALSE, fig.cap=""}
climatedf_comp %>% 
  ggplot() +
