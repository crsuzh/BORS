---
source: Rmd
title: "Week 3 Reproducibility and Data"
teaching: 0
exercises: 0
questions: 
  - "tbd"
...
# 031-3-organizationandsoftware: 2. Organization and software
...
# 031-4-datainspreadsheets: 3. Data in spreadsheets
...
# 031-5-RStudio: 4. Some practical tips for the use of Rstudio (optional)


## RStudio
Using Rstudio is not the only way how you can use R. In this course we use it because it offers many conveniences such as

* easy handling of analysis scripts 
* the console (which runs R)
* the terminal to directly interact with the file system
* output (such as plots)
* the possibility to handle version controlled repositories
* other utilities 

Here we give a few helpful tips.

The **top menu bar** has multiple tabs containing many commands that are frequently used. The ones we will be using are mostly located within the `File` and `Session` sections. In `File` two useful commands are for opening existing files and creating new files. In `Session` the command `Restart R` is very useful to **clean up your session**, e.g. remove variables that have been created but are not used anymore, which can be very important to check if your script works properly.  

Rstudio is per default split into four panes which can be adjusted to your preference.   The pane in the top left **Top left Pane: Scripts** shows your open scripts (hidden if you don't have any open scripts) . This is where you write your analysis.  

The pane on the bottom left **Bottom left Pane: console** is the console . This is where R is run and your commands are executed. Furthermore you have the option to open a terminal (i.e. a command line interface to your computer) by selecting the tab `Terminal`.   

The pane on the top right **Top right Pane: Environment** contains among others information about the environment (e.g. the objects you store in your R session). 

The pane in the bottom right **Bottom right Pane: Plots, Help, Files **  contains another set of utilities, with the tab names `Files`, `Plots`, `Packages`, `Help` and `Viewer` . `Files` shows the local directory and files (on your copmuter) and can be used as a file manager with possible operations such as opening, renaming, deleting, creation of folders. `Plots` shows the plots (or in general graphics) created in the Console. `Packages` shows the list of installed packages and an indication for each package if it is currently loaded (packages are collections of functions and data set developed by the community, one package usually contains functionality for a specific purpose extending the basic functionality of R). Additionally packages can be installed and updated. `Help` shows the R help pages and function references, e.g. run `?plot` in the console. `Viewer` allows to - as the name suggests - view different output such as html pages.

![]({{ page.root }}/fig/06-RStudio_example.png)
...
* `Tools` > `Global` options
  - under `General` > `Workspace` make sure *Never* is selected in `Save workspace to .RData on exit`
  - change `Appearance` to your liking
  - change `Pane Layout` to your liking
* `Tools`
  - `Keyboard Shortcuts Help`
* `Help`
  - `Markdown Quick Reference` (we will use Markdown next week)
  - `Cheat Sheets` > `RStudio IDE Cheat Sheet`
  - `Cheat Sheets` > `R Markdown Cheat Sheet`
...
You will upload the resulting improved file in the next step (allowed file formats are xls, xlsx, csv, ods, fods, uos). Your submission will be peer reviewed. Staff will perform random checks on the peer review.
...
**Upload the complete script** file containing at least Tasks 1--2 in the next step, if you have a specific question on your code put this question in the textbox.
